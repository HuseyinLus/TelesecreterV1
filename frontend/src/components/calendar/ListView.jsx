      ...
      {filtered.map((a) => {
        ...
        return (
          <button
            key={a.id}
            onClick={() => onSelect(a)}
            style={{ display: 'grid', gridTemplateColumns: COLS, gap: 14, padding: '10px 18px', alignItems: 'center', width: '100%', textAlign: 'left', fontSize: 12.5, borderBottom: '1px solid var(--color-border)', borderLeft: today ? '3px solid var(--color-accent)' : '3px solid transparent', background: selectedId === a.id ? 'var(--color-accent-soft)' : 'transparent', transition: 'background 0.1s', cursor: 'pointer', border: 'none' }}
          >

...
export default function DayView({ appointments, doctors, filters, selectedId, onSelect, baseDate }) {
  const todayDate = new Date().toISOString().slice(0, 10)
  const viewDate = (baseDate ?? new Date()).toISOString().slice(0, 10)
  const activeDoctors = doctors.filter((d) =>
    filters.doctorId === 'all' || d.id === filters.doctorId
  )
  const filtered = appointments.filter((a) => {
    if (a.date !== viewDate) return false
        ...
        {activeDoctors.map((d) => {
          ...
          return (
            <div key={d.id} style={{ position: 'relative', borderLeft: '1px solid var(--color-border)', minHeight: HOUR_HEIGHT * GRID_HOURS.length }}>
              ...
              {viewDate === todayDate && <NowIndicator />}
              {colAppts.map((a) => {
                ...
                return (
                  <button key={a.id} onClick={() => onSelect(a)} style={{ position: 'absolute', left: 4, right: 4, top: topPx, height: heightPx, borderRadius: 7, background: c.bg, border: `1px solid ${c.stripe}30`, display: 'flex', overflow: 'hidden', cursor: 'pointer', boxShadow: selectedId === a.id ? `0 0 0 2px var(--color-accent)` : 'none' }}>
